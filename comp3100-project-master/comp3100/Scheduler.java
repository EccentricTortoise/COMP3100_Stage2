...
public class Scheduler {
    ...
    public static void scheduleJob(Connection conn, String[] args) throws Exception {
        ...
        // Commented out for getCapable. Fully functional.
        ...
        Server bestServer = getCapable(conn, cores, memory, disk)[0];
    ...
    /*private static Server[] makeServerList(String input, int numServers) {
        Server[] val = new Server[numServers];
    ...
    private static Server[] getCapable(Connection conn, int core, int memory, int disk) {
        try {
            conn.send("GETS Capable " + core + " " + memory + " " + disk);
            ...
            for (int i = 0; i < numServers; i++) {
                ...
                int cores = Integer.parseInt(data1[4]);
                int mem = Integer.parseInt(data1[5]);
                int disk1 = Integer.parseInt(data1[6]);
                servers[i] = new Server(type, id, cores, mem, disk1, 0, 0, 0, state, 0);
            }
